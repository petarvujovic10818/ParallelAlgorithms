      ...
      __global__ void multiply(float * A, float * B, float * C,
          ...
          for (int i = 0; i < (numAColumns-1)/TILE_WIDTH+1; i++) {
            if (Row < numARows && i*TILE_WIDTH+tx < numAColumns)
                ds_M[ty][tx] = A[Row*numAColumns + i*TILE_WIDTH+tx];
            else
                ds_M[ty][tx] = 0;
            if (Col < numBRows && i*TILE_WIDTH+ty < numBColumns)
                ds_N[ty][tx] = B[Col*numBColumns + i*TILE_WIDTH+ty];
            else
                ds_N[ty][tx] = 0;
